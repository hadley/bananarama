...
test_that("parse_defaults extracts values from named list", {
  ...
  expect_equal(result$model, "gemini-3-pro-image")
...
test_that("parse_image merges defaults with overrides", {
  ...
  expect_equal(result$model, "gemini-3.1-flash-lite-image")
...
test_that("parse_image errors on unsupported model", {
  defaults <- parse_defaults(NULL)
  img <- list(name = "test", description = "desc", model = "dall-e-3")
  expect_error(parse_image(img, defaults), "unsupported")
})

test_that("parse_image looks up provider from model", {
  defaults <- parse_defaults(NULL)
  result <- parse_image(list(name = "test", description = "desc"), defaults)
  expect_equal(result$provider, "gemini")

  result <- parse_image(
    list(name = "test", description = "desc", model = "gpt-image-2.5-flare"),
    defaults
  )
  expect_equal(result$provider, "openai")
})

test_that("parse_image warns and drops seed for OpenAI models", {
  defaults <- parse_defaults(NULL)
  img <- list(
    name = "test",
    description = "desc",
  ...
  expect_warning(result <- parse_image(img, defaults), "seed")
